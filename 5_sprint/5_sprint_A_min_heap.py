...
Такой вариант рабочий, но приходится при печати результата разворачивать итоговый список. Чтобы этого не делать, перепишем алгоритм на вариант, когда в голову кучи выносится наименьший элемент - min_heap.
...
3) Итеративно перебираем все элементы неотсортированной кучи. Каждый элемент добавляем на те позиции, которые являются дочерними по отношению к корню. Причем при добавлении сравниваем добавляемый элемент с родительским.
...
"""
Применяется метод сортировки min_heap, что гарантирует время выполенения всей сортировки за O(nlog n)
"""
