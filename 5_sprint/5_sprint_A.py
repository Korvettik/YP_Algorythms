...
def shift_up(heap, index):
    ...
    if compare(heap[index], heap[parent_index]) > 0:  # сравниваются кортежи
...
def shift_down(heap, index):
    ...
    if right <= heap_max_index and compare(heap[right], heap[left]) > 0:  # если индекс справа существует + значение правого ребенка больше левого
    ...
    if compare(heap[index_largest], heap[index]) > 0:
...
def compare(a, b):
    """
    Сравнивает два кортежа (score, penalty, name) по правилам:

    Сначала по score (больше - лучше)
    При равных score по penalty (меньше - лучше)
    При равных score и penalty по name (больше в лексикографическом порядке - лучше)
    """
    # Сравниваем score
    if a[0] > b[0]:
        return 1
        ...
        # Score равны, сравниваем penalty
        if a[1] < b[1]:
            return 1
        elif a[1] > b[1]:
            return -1
        else:
            # Score и penalty равны, сравниваем name
            if a[2] < b[2]:
                return 1
            elif a[2] > b[2]:
                return -1
