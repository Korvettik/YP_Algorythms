# https://contest.yandex.ru/contest/25597/run-report/154311798/
...
Пространственная сложность - O(n*m), т.к. мы храним значения в ячейках матрицы, а матрица имеет размеры n*m
"""
    ...
    for i in range(1, len_s + 1):
        for j in range(1, len_t + 1):
            ...
            if s[i - 1] == t[j - 1]:
                ...
                matrix[i][j] = 1 + min(
                    matrix[i - 1][j],  # удаление (если бы не было этой разницы, то взяли бы значение от верхней диагонали, т.е. удалили бы данный элемент)
                    matrix[i][j - 1],  # вставка (случай, если нам проще добавить этот элемент, то берем значение из нижней диагонали)
                    matrix[i - 1][j - 1]  # замена (если нам проще поменять элемент, то берем текущую диагональ)
                )
