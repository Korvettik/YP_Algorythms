...
def merge_sort(arr, lf, rg):
    ...
    # Базовый случай: если в подмассиве 1 элемент или меньше, он уже отсортирован (отсюда пойдет обратный ход)
    if rg - lf <= 1:
        return
    ...
    # обратным ходом будем возвращаться сюда и идти далее на слияние merge.
    # При этом группы сравнения укрупняются (индексы шире), но внутри они частично (из-за добавления остатков) уже отсортированы
